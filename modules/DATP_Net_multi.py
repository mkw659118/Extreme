...
class ChannelwiseStudentTMixturePrior(nn.Module):
    def __init__(
        self,
        num_components: int,
        state_dim: int,
        num_channels: int,
        value_groups_per_channel: int = 1,
        use_all_channels: bool = True,
        include_last_value: bool = True,
        scales: Sequence[int] = (1, 4, 8, 16),
        include_seq_level: bool = True,
        ...
        self.value_groups_per_channel = max(1, int(value_groups_per_channel))
        self.use_all_channels = use_all_channels
        self.include_last_value = include_last_value
        self.scales = tuple(scales)
        self.include_seq_level = include_seq_level
        ...
        self.mu = nn.Parameter(torch.zeros(self.num_channels, num_components, state_dim))
        self.scale_raw = nn.Parameter(torch.zeros(self.num_channels, num_components, state_dim))
        self.df_raw = nn.Parameter(torch.zeros(self.num_channels, num_components, state_dim))
        self.mix_logits = nn.Parameter(torch.zeros(self.num_channels, num_components))
        self.register_buffer('_log_pi', torch.log(torch.tensor(torch.pi, dtype=torch.float32)), persistent=False)
    ...
    def reset_parameters(self):
        with torch.no_grad():
            ...
            if self.num_components > 1:
                comp_offsets = torch.linspace(-0.5, 0.5, steps=self.num_components, device=self.mu.device)
                self.mu[:, :, 0] = self.mu[:, :, 0] + comp_offsets.view(1, -1)

                scale_offsets = torch.linspace(-0.12, 0.12, steps=self.num_components, device=self.scale_raw.device)
                df_offsets = torch.linspace(-0.08, 0.08, steps=self.df_raw.size(1), device=self.df_raw.device)
                self.scale_raw.add_(scale_offsets.view(1, -1, 1))
                self.df_raw.add_(df_offsets.view(1, -1, 1))

            self.scale_raw.add_(0.01 * torch.randn_like(self.scale_raw))
            self.df_raw.add_(0.01 * torch.randn_like(self.df_raw))
            nn.init.normal_(self.mix_logits, mean=0.0, std=0.05)

    def _reshape_channel_groups(self, x: torch.Tensor) -> torch.Tensor:
        if not self.use_all_channels:
            x = x[:, :, :1]

        batch_size, length, features = x.shape
        expected = self.num_channels * self.value_groups_per_channel

        if features == expected:
            return x.view(batch_size, length, self.num_channels, self.value_groups_per_channel)

        if features == self.num_channels:
            return x.view(batch_size, length, self.num_channels, 1)

        if features % self.num_channels == 0:
            groups = features // self.num_channels
            return x.view(batch_size, length, self.num_channels, groups)

        raise ValueError(
            f"Cannot reshape state-prior input with {features} features into "
            f"{self.num_channels} channels."
        )

    def extract_state_vector(self, x: torch.Tensor) -> torch.Tensor:
        # x: [B, L, C * G] -> z: [B, C, D]
        x_used = self._reshape_channel_groups(x)
        std = x_used.std(dim=1, unbiased=False).mean(dim=-1, keepdim=True)
    ...
    def component_diversity_loss(
        ...
        scale_sep_loss = F.relu(scale_margin - log_scale_dist).pow(2).mean()
        df_sep_loss = F.relu(df_margin - log_df_dist).pow(2).mean()
        diversity_loss = mu_sep_loss + scale_weight * scale_sep_loss + df_weight * df_sep_loss
    ...
    def posterior_from_z(self, z: torch.Tensor) -> Dict[str, torch.Tensor]:
        log_comp = self._component_log_prob(z)
        log_pi = torch.log_softmax(self.mix_logits, dim=-1).unsqueeze(0)
        log_joint = log_comp + log_pi
        log_mix = torch.logsumexp(log_joint, dim=-1)
        q_channel = torch.softmax(log_joint / self.temperature, dim=-1)
        return {
            'log_component': log_comp.mean(dim=1),
            'log_component_channel': log_comp,
            'log_joint': log_joint.mean(dim=1),
            'log_joint_channel': log_joint,
            'log_mix': log_mix.mean(dim=1),
            'log_mix_channel': log_mix,
            'q': q_channel.mean(dim=1),
            'q_channel': q_channel,
            'mix_prob': torch.softmax(self.mix_logits, dim=-1).mean(dim=0),
            'mix_prob_channel': torch.softmax(self.mix_logits, dim=-1),
        }
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        for patch_len in self.scales:
            ...
            log_pi = torch.log_softmax(self.mix_logits, dim=-1).unsqueeze(0)
            q_channel_scale = torch.softmax((log_comp_scale + log_pi) / self.temperature, dim=-1)
            q_channel_scales.append(q_channel_scale)
            q_scales.append(q_channel_scale.mean(dim=1))
        ...
        if self.include_seq_level:
            ...
            log_pi = torch.log_softmax(self.mix_logits, dim=-1).unsqueeze(0)
            q_channel_seq = torch.softmax((log_comp_seq + log_pi) / self.temperature, dim=-1)
            q_channel_scales.append(q_channel_seq)
            q_scales.append(q_channel_seq.mean(dim=1))

        alpha = torch.softmax(self.alpha_logits, dim=0)
        log_comp_stack = torch.stack(log_comp_scales, dim=1)  # [B, S, C, K]
        fused_log_comp = torch.sum(log_comp_stack * alpha.view(1, -1, 1, 1), dim=1)

        log_pi = torch.log_softmax(self.mix_logits, dim=-1).unsqueeze(0)
        log_joint = fused_log_comp + log_pi
        log_mix_channel = torch.logsumexp(log_joint, dim=-1)
        q_channel = torch.softmax(log_joint / self.temperature, dim=-1)

        z_channel = torch.stack(z_scales, dim=1).mean(dim=1)
        out = {
            'log_component': fused_log_comp.mean(dim=1),
            'log_component_channel': fused_log_comp,
            'log_joint': log_joint.mean(dim=1),
            'log_joint_channel': log_joint,
            'log_mix': log_mix_channel.mean(dim=1),
            'log_mix_channel': log_mix_channel,
            'q': q_channel.mean(dim=1),
            'q_channel': q_channel,
            'mix_prob': torch.softmax(self.mix_logits, dim=-1).mean(dim=0),
            'mix_prob_channel': torch.softmax(self.mix_logits, dim=-1),
            'z': z_channel.mean(dim=1),
            'z_channel': z_channel,
            'z_scales': torch.stack(z_scales, dim=1),
            'q_scales': torch.stack(q_scales, dim=1),
            'q_channel_scales': torch.stack(q_channel_scales, dim=1),
            'alpha': alpha,
        }
        out['pretrain_nll'] = -log_mix_channel.mean()
        return out
...
class ExtremeLSTMMemo(nn.Module):
    def __init__(
        ...
        self.dec_in = dec_in
        self.out_dim = out_dim
        self.num_vars = int(getattr(self.config, 'num_vars', out_dim))
        ...
        default_value_groups = 3 if self.num_vars > 0 and c_in % self.num_vars == 0 and c_in // self.num_vars in {3, 6} else 1
        state_prior_value_groups = int(
            getattr(self.config, 'state_prior_value_groups_per_channel', default_value_groups)
        )
        self.state_prior = ChannelwiseStudentTMixturePrior(
            num_components=self.num_states,
            state_dim=state_dim,
            num_channels=self.num_vars,
            value_groups_per_channel=state_prior_value_groups,
    ...
    def _forward_backbone(self, x: torch.Tensor):
        ...
        if self.use_state_prior:
            ...
            state_probs_channel = prior_out.get('q_channel')
            state_z = prior_out['z']
        ...
        backbone_out.update({
            ...
            'state_probs_channel': state_probs_channel,
            'state_z': state_z,
