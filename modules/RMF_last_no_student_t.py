...
class RouterFromEmbedding(nn.Module):
    def __init__(self, d_model: int, num_experts: int, hidden: int = 64, dropout: float = 0.0):
        super().__init__()
        in_dim = d_model * 3
    ...
    def forward(self, x_emb: torch.Tensor) -> torch.Tensor:
        feat_mean = x_emb.mean(dim=1)
        feat_std = x_emb.std(dim=1, unbiased=False)
        feat_last = x_emb[:, -1, :]
        feat = torch.cat([feat_mean, feat_std, feat_last], dim=-1)
...
class ExtremeLSTMMemo(nn.Module):
    ...
    def _forward_backbone(self, x: torch.Tensor):
        ...
        router_logits = self.router(x_emb)
        router_prob = torch.softmax(router_logits, dim=-1)
