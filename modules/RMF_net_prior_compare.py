...
class GaussianMixturePrior(StudentTMixturePrior):
    def __init__(
        self,
        num_components: int,
        state_dim: int,
        ...
        super().__init__(
            ...
            scales=scales,
            include_seq_level=include_seq_level,
            learnable_scale_weights=learnable_scale_weights,
            min_scale=min_scale,
            min_df=2.1,
        ...
        self.register_buffer(
            '_log_two_pi',
            torch.log(torch.tensor(2.0 * torch.pi, dtype=torch.float32)),
            persistent=False,
        )
