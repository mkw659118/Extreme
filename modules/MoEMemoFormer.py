...
#Author  :   mkw
#Time    :   2025/09/17 10:50:52
#Desc    :   PatchExtremeMemoryTransformer (Scheme A)
#           Shared Backbone + Soft Mixture Heads (sample-level routing as weights)
#           + Step Embedding for pred tokens
#           + Horizon-Weighted Loss helper
...
# =========================================================
# Router (sample-level) from RAW x
# =========================================================
class SampleRouterFromX(nn.Module):
    def __init__(self, c_in: int, num_experts: int, hidden: int = 128, dropout: float = 0.0):
        super().__init__()
...
# =========================================================
# Diversity regularizer for heads
# =========================================================
def head_diversity_loss(expert_heads: nn.ModuleList, eps: float = 1e-8):
    """
    For Linear head: weight shape [1, d_model]
...
# =========================================================
# Horizon-weighted loss helper (TRAINING SIDE)
# =========================================================
def horizon_weighted_loss(
    pred: torch.Tensor,
    target: torch.Tensor,
    base: str = "mse",     # "mse" or "mae"
    gamma: float = 2.0,    # >1 强化远期
    eps: float = 1e-6,
):
    """
    pred/target: [B, pred_len, 1] (or [B, pred_len, D])
    weight(t) = (t/(T-1) + eps)^gamma , then normalized by mean to keep loss scale stable
    """
    assert pred.shape[:2] == target.shape[:2], "pred/target must match on [B, T]"
    B, T = pred.shape[0], pred.shape[1]

    t = torch.arange(T, device=pred.device, dtype=pred.dtype)
    w = (t / max(T - 1, 1) + eps).pow(gamma)  # [T]
    w = w / w.mean().clamp_min(eps)          # normalize scale

    # reshape to broadcast: [1, T, 1, 1, ...]
    shape = [1, T] + [1] * (pred.dim() - 2)
    w = w.view(*shape)
...
class ThreeExpertPatchTransformer(nn.Module):
    """
    Scheme A changes:
      1) Soft mixture of experts (no hard Top1 gather):
         y = sum_e p(e|x) * head_e(shared)
      2) Step embedding for pred tokens:
         pred_token[t] = base + Emb(step=t)
      3) Keep router CE/balance/head_div as aux losses

    Notes:
      - route_labels from GMM are mapped to expert id by label2expert
      - output is [B, pred_len, 1]
    """
    ...
    def __init__(
        ...
        # -------- Scheme A: pred tokens with step embedding --------
        # base token: [1, d_model]
        self.pred_base = nn.Parameter(torch.randn(1, d_model))
        # step embedding: [pred_len, d_model]
        self.pred_step = nn.Embedding(pred_len, d_model)
        ...
        # label2expert[k] = GMM标签k 路由到的 expert id
        label2expert = getattr(config, "label2expert", None)
        if label2expert is None:
            label2expert = list(range(self.num_experts))
        self.register_buffer("label2expert", torch.tensor(label2expert, dtype=torch.long))
    ...
    def _gmm_argmax_label(self, x: torch.Tensor) -> torch.Tensor:
        ...
        label = torch.argmax(gmm_mean, dim=-1)    # [B] in {0..E-1}
        return label
    ...
    def forward(self, x, x_mark=None, y_true=None, sample_ids=None, route_labels=None):
        ...
        # gmm labels -> expert labels (for supervision only)
        if route_labels is None:
            route_labels = self._gmm_argmax_label(x)    # [B] in gmm-label space
        expert_labels = self.label2expert[route_labels] # [B] in expert-id space

        # ---------------- embedding + pred tokens (step embedding) ----------------
        x_emb_hist = self.embedding(x)                  # [B, seq_len, d_model]

        step_ids = torch.arange(self.pred_len, device=x.device, dtype=torch.long)  # [pred_len]
        pred_token = self.pred_base + self.pred_step(step_ids)                     # [pred_len, d_model]
        pred_token = pred_token.unsqueeze(0).expand(B, -1, -1)                     # [B, pred_len, d_model]
        ...
        # ---------------- Scheme A: soft mixture output ----------------
        y = (y_all * router_prob.unsqueeze(1)).sum(dim=-1, keepdim=True)           # [B, pred_len, 1]

        # ---------------- aux losses ----------------
        aux_loss = {}
        if self.w_router_ce > 0.0 and self.training:
            aux_loss["router_ce"] = self.w_router_ce * F.cross_entropy(router_logits, expert_labels)
