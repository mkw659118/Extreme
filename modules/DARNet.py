...
class ExtremeLSTMMemo(nn.Module):
    ...
    def cosine_similarity(self, queries: torch.Tensor, keys: torch.Tensor) -> torch.Tensor:
        ...
        Original code computed cosine similarity over the full flattened input,
        which would treat missing placeholders as normal values. Here we remove
        mask channels from the retrieval vector and multiply each value channel
        by its own validity mask before normalization. Therefore invalid
        first-diff / second-diff / raw positions do not directly contribute to
        ...
        if len(queries.shape) == 3:
            ...
            q_values = (q_values * q_masks).reshape(batch_size, -1)
            k_values = (k_values * k_masks).reshape(num_keys, -1)

            q_norm = q_values / (torch.norm(q_values, p=2, dim=-1, keepdim=True) + eps)
            k_norm = k_values / (torch.norm(k_values, p=2, dim=-1, keepdim=True) + eps)
