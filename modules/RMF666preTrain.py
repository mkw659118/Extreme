...
class StudentTMixturePrior(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        z = self.extract_state_vector(x)
        out = self.posterior_from_z(z)
        out['z'] = z
        out['pretrain_nll'] = -out['log_mix'].mean()
        return out
...
class RouterFromEmbeddingPreTrain(nn.Module):
    ...
    def forward(self, x_emb: torch.Tensor, q_prior: torch.Tensor) -> torch.Tensor:
        emb_mean = x_emb.mean(dim=1)
        emb_std = x_emb.std(dim=1, unbiased=False)
        emb_last = x_emb[:, -1, :]
        feat = torch.cat([emb_mean, emb_std, emb_last, q_prior], dim=-1)
...
class ExtremeLSTMMemo(nn.Module):
    ...
    def _forward_backbone(self, x: torch.Tensor):
        ...
        prior_out = self.state_prior(x)
        router_logits = self.router(x_emb, prior_out['q'])
        router_prob = torch.softmax(router_logits, dim=-1)
