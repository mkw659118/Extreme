...
# ========== 核心修正：标准MoE专家（输出维度=输入维度d_model） ==========
class MoEExpert(nn.Module):
    """标准MoE专家：仅做特征变换，输出维度保持d_model"""
    def __init__(self, d_model: int, hidden: int = None, dropout: float = 0.3):
        super().__init__()
        hidden = hidden or (2 * d_model)
        self.fc1 = nn.Linear(d_model, hidden)
        self.fc2 = nn.Linear(hidden, hidden)
        self.fc3 = nn.Linear(hidden, d_model)  # 输出维度=输入维度d_model
        self.act = nn.GELU()
...
class StandardMoEHead(nn.Module):
    ...
    def forward(self, x, router_probs, topk_experts):
        """
        Args:
            x: [B, pred_len, d_model] - 编码器/解码器输出特征
            router_probs: [B, top_k] - 归一化后的top-k专家权重
            topk_experts: [B, top_k] - top-k专家索引
        Returns:
            out: [B, pred_len, 1] - 最终预测结果
        """
