...
class PatchExtremeMemoryTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.use_extreme_gate = use_extreme_gate
        ...
        if self.share_weights:
            self.intra_patch_blocks_shared = nn.ModuleList([
                TransformerBlock(d_model, num_heads, d_ff=d_ff, dropout=dropout)
                for _ in range(num_layers_intra_patch)
            ])
        else:
    ...
    def forward(self, x, x_mark=None, sample_ids=None):
        """
        x: [B, seq_len, 1]
        sample_ids: [B] LongTensor（样本唯一ID）；可为 None（退化为全局检索）
        return y: [B, pred_len, 1]
        """
        B, L, C = x.shape
        assert L == self.seq_len and C == 1
        ...
        # =====================================================
        if self.share_weights:
            intra = rearrange(patches, 'b p pl d -> (b p) pl d')
            for block in self.intra_patch_blocks_shared:
                intra = block(intra, attn_mask=intra_patch_mask)
            intra_tokens = rearrange(intra, '(b p) pl d -> b (p pl) d', b=B, p=self.num_patches)
        else:
            outs = []
            for p in range(self.num_patches):
                out = patches[:, p, :, :]
                ...
                outs.append(out)
            intra_tokens = torch.cat(outs, dim=1)  # [B,P*pl,d]

        # =====================================================
        # 分支 B：Inter-branch + 样本级记忆
        # =====================================================
        inter_patches = patches  # [B,P,pl,d]

        if self.use_memory:
            # 1) 先得到每个 patch 的代表（沿 pl 平均）
            q_patch = patches.mean(dim=2)  # [B,P,d]

            # 2) 以极端掩码加权，构造样本级查询 q_s；若某样本无极端，则用均匀权重
            ext_w = extreme_mask.float()                     # [B,P]
            has_ext = ext_w.any(dim=1, keepdim=True)         # [B,1]
            safe_w = torch.where(has_ext, ext_w, torch.ones_like(ext_w))
            safe_w = safe_w / (safe_w.sum(dim=1, keepdim=True) + 1e-6)
            q_s = (safe_w.unsqueeze(-1) * q_patch).sum(dim=1)  # [B,d]

            # 3) 从样本库检索
            if sample_ids is None:
                fake_ids = torch.full((B,), -1, device=q_s.device, dtype=torch.long)
                m_read, sim_max, _ = self.memory.read(fake_ids, q_s)
            else:
                m_read, sim_max, _ = self.memory.read(sample_ids, q_s)  # [B,d], [B,1]

            # 4) 门控融合得到样本级基准向量 q_base
            gate_s = torch.sigmoid(self.gate_proj(torch.cat([q_s, sim_max], dim=-1)) + self.gate_bias)  # [B,d]
            fuse_s = self.mem_fuse(torch.cat([q_s, m_read], dim=-1)) * self.mem_scale                   # [B,d]
            q_base = (1.0 + gate_s) * q_s + gate_s * fuse_s                                             # [B,d]

            # 5) 只对极端 patch 注入样本级增量：ext=1 用 q_base，ext=0 保留 q_patch
            q_base_exp = q_base.unsqueeze(1).expand(-1, self.num_patches, -1)  # [B,P,d]
            ext_m = extreme_mask.unsqueeze(-1).float()                          # [B,P,1]
            q_new_all = ext_m * q_base_exp + (1.0 - ext_m) * q_patch            # [B,P,d]

            delta = (q_new_all - q_patch).unsqueeze(2)                           # [B,P,1,d]
            inter_patches = patches + delta.expand(-1, -1, self.patch_len, -1)   # [B,P,pl,d]

            # 6) 训练期：样本级写入（仅当该样本存在极端 patch）
            if self.training:
                owner_ids = sample_ids if sample_ids is not None else torch.full((B,), -1, device=q_s.device, dtype=torch.long)
                write_mask = extreme_mask.any(dim=1)  # [B]
                if write_mask.any():
                    self.memory.write(
                        owner_ids[write_mask].detach(),
                        k_batch=F.normalize(q_s[write_mask].detach(), dim=-1),
                        v_batch=F.normalize(q_base[write_mask].detach(), dim=-1)
                    )
        ...
        # ---------- 输出 ----------
        y = self.proj_out(final)        # [B,total_len,1]
        y = y[:, -self.pred_len:, :]    # [B,pred_len,1]
