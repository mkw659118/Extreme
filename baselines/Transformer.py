...
class Transformer(torch.nn.Module):
    def __init__(self, input_size, d_model, revin, num_heads, num_layers,
        ...
        self.patch_inter_blocks = torch.nn.ModuleList([
            torch.nn.Sequential(
                get_norm(d_model, norm_method),
                get_att(d_model, num_heads, att_method),
                get_norm(d_model, norm_method),
    ...
    def forward(self, x, x_mark=None):
        ...
        patch_mask, inter_mask = self._build_masks(x.device, x.dtype)
        ...
        # 
        x_patch_tokens = x_patch_tokens.mean(dim=2)

        out_tokens = x_patch_tokens
        for block in self.patch_inter_blocks:
            norm1, attn, norm2, ff = block
            if self.att_method == 'self':
        ...
        x_patch_ctx = out_tokens.unsqueeze(2).repeat(1, 1, self.patch_len, 1)
        x = rearrange(x, 'b (np pl) d -> b np pl d', np=self.num_patches, pl=self.patch_len)
        x = x + x_patch_ctx
        x = rearrange(x, 'b np pl d -> b (np pl) d')
