...
class DS:
    ...
    Label Y is first-order diff normalized [N, pred_len, C], so all variables
    are predicted together while the original recover_level_from_diff()
    ...
    def _load_csv(self) -> np.ndarray:
        ...
        setattr(self.config, "dec_in", self.target_dim)
        setattr(self.config, "out_dim", self.target_dim)
        setattr(self.config, "target_col", self.target_col)
        setattr(self.config, "target_dim", self.target_dim)
        ...
        if self.target_dim != self.num_vars:
            raise ValueError(
                f"DATP-Net multi predicts all variables, but target_dim={self.target_dim} "
    ...
    def _apply_random_artificial_missing(
        ...
        if target_only:
            # Every original variable is a prediction target in this multi
            # dataset, so target_only keeps all observed variable positions.
    ...
    def _make_windows(
        ...
        for start in range(0, total, self.stride):
            ...
            y = label_diff_norm[y_start:y_end, :]
            y_mask = label_diff_mask[y_start:y_end, :]
    ...
    def inverse_diff_norm(self, diff_norm: np.ndarray) -> np.ndarray:
        ...
        if diff_norm.shape[-1] == self.num_vars:
            mean = self.mean
            std = self.std
        elif diff_norm.shape[-1] == 1:
            mean = self.mean[self.target_col : self.target_col + 1]
            std = self.std[self.target_col : self.target_col + 1]
        else:
            raise ValueError(
                f"Cannot inverse-normalize diff with last dim {diff_norm.shape[-1]}; "
    ...
    def recover_level_from_diff(
        ...
        if diff_norm.shape[-1] == self.num_vars:
            ...
            raise ValueError(
                f"Cannot recover level for last dim {diff_norm.shape[-1]}; "
                f"expected 1 or num_vars={self.num_vars}."
            )
