...
#Desc    :   Data processing with SMM (StudentTMixture) instead of GMM
...
# ---- NEW: 引入 StudentTMixture 代替 GaussianMixture ----
from modules.student_mixture import StudentTMixture
...
class DS2:
    ...
    def __init__(self, config, trainX):
        ...
        # ---- NEW: 使用 SMM (StudentTMixture) 替代 GMM，用于异常检测 ----
        # df 可以调，例如 df=5.0 或 df=7.0；越小尾越重
        self.gm3 = StudentTMixture(n_components=3, df=2.0)
        ...
        if self.config.mode == "train":
            self.train_temp_gmm()         # 临时训练 SMM (之前是 GMM)
            ...
            print("样本第0列：", self.sensor_data_norm1[:10, 0])  # 异常概率 / 原始特征
    ...
    def read_dataset(self):
        ...
        # 使用随机采样数据训练 SMM
        self.gmm0 = StudentTMixture(n_components=3, df=2.0)
        series = []
    ...
    def train_temp_gmm(self):
        """临时训练 SMM，供验证集生成时使用，增强版：解决样本不足问题"""
        # 1. 提取原始数据并清洗NaN值
        ...
        min_required_samples = 2  # SMM 训练至少需要2个样本（这里沿用你原来的逻辑）
        ...
        if n_samples < min_required_samples:
            print(f"警告：有效样本数不足（{n_samples}个），将复制样本以满足SMM训练要求")
            temp_gmm_input = []
            ...
            while len(temp_gmm_input) < min_required_samples:
                temp_gmm_input.append(temp_gmm_input[-1])
        ...
        # 4. 训练临时 SMM 并保存
        self.gmm = StudentTMixture(n_components=3, df=2.0)
        self.gmm.fit(temp_gmm_input)
        torch.save(self.gmm, self.expr_dir + "/" + "GMM.pt")
        print(f"临时SMM训练完成，使用了{len(temp_gmm_input)}个样本（窗口大小{window_size}）")
