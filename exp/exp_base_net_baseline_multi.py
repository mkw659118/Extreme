...
from exp.exp_loss_net_baseline_multi import compute_loss
...
class BasicModel(torch.nn.Module):
            ...
            for batch_idx, train_batch in enumerate(dataModule.train_data_loader):
                ...
                if self.config.use_amp:
                    with torch.autocast(device_type=self.device_type, dtype=torch.float16):
                        ...
                        pred_scaled = pred[:, -self.pred_len:, :]
                        real_scaled = label[:, -self.pred_len:, :]
    ...
    def _build_test_save_paths(self):
        ...
        target_tag = "ALL" if str(getattr(self.config, "target_cols", "")).lower() == "all" else str(original_target_col)
        draw_root = getattr(self.config, "draw_root", "./draw_multi")
        result_prefix = (
            f"{model_name}_{data_tag}_"
            f"PL{self.config.pred_len}_DM{self.config.d_model}_TC{target_tag}"
        )
        ...
        return (
            os.path.join(save_dir, f"{result_prefix}_window_raw.csv"),
            os.path.join(save_dir, f"{result_prefix}_agg_raw.csv"),
        )
