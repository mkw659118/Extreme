                ...
                if self.config.use_amp:
                    ...
                    if (batch_idx + 1) % 100 == 0 or (batch_idx + 1) == len(dataModule.train_data_loader):
                        ...
                        if hasattr(self.model, 'latest_aux_dict'):
                            if 'beta_mean' in self.model.latest_aux_dict:
                                beta_mean_item = float(self.model.latest_aux_dict['beta_mean'].item())
                            if 'state_probs' in self.model.latest_aux_dict:
                                q = self.model.latest_aux_dict['state_probs'].mean(dim=0)
                                q_mean_str = f", Q=[{q[0].item():.3f}, {q[1].item():.3f}, {q[2].item():.3f}]"
                            if 'state_alpha' in self.model.latest_aux_dict:
                                a = self.model.latest_aux_dict['state_alpha']
                                alpha_str = f", alpha={['%.2f' % v for v in a.tolist()]}"
