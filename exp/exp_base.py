...
class BasicModel(torch.nn.Module):
    ...
    def _maybe_inverse_std(self, reals, preds, dataModule):
        if not hasattr(dataModule, 'mean') or not hasattr(dataModule, 'std'):
            return reals, preds
        ...
        reals_np = inverse_std_normalization(reals_np, mean, std)
        preds_np = inverse_std_normalization(preds_np, mean, std)
        return reals_np, preds_np
    ...
    def valid(self, dataModule):
        ...
        reals, preds = self._maybe_inverse_std(reals, preds, dataModule)
        return ErrorMetrics(reals, preds, self.config, 'valid')
    ...
    def test(self, dataModule):
        ...
        preds = torch.cat(preds, dim=0)
        reals, preds = self._maybe_inverse_std(reals, preds, dataModule)
        return ErrorMetrics(reals, preds, self.config, 'test')
