...
class BasicModel(torch.nn.Module):
    def __init__(self, config):
        ...
        # Ablation: w/o Retrieval Bank.
        # Gate stage is removed, so gate scaler is no longer used.
    ...
    def setup_gate_optimizer(self, config):
        ...
        RetrievalBetaGate has been removed.
        This function is kept as a no-op to avoid old scripts crashing
        if they accidentally call setup_gate_optimizer().
        """
        self.gate_optimizer = None
        self.gate_scheduler = None
        print("[Ablation] setup_gate_optimizer skipped because retrieval gate is disabled.")
    ...
    def pretrain_one_epoch(self, dataModule):
        ...
        print(
            ...
            f"Avg NLL: {avg_loss:.6f} | "
            f"Time Cost: {t2 - t1:.2f}s"
        )

        return avg_loss, t2 - t1, last_aux

    def prepare_retrieval_index(self, train_data=None, train_loader=None):
        """
        Ablation: w/o Retrieval Bank.

        Retrieval memory bank is removed.
        Therefore, no key-value index is constructed.
        This function is kept as a no-op for compatibility.
        """
        print("[Ablation] Retrieval index construction skipped.")
        return
    ...
    def train_one_epoch(self, dataModule, stage='backbone'):
        ...
        In the original full model:
            stage='backbone' trains the forecasting backbone.
            stage='gate' trains the retrieval beta gate.

        In this ablation:
            Retrieval gate is removed.
            Any accidental stage='gate' call is redirected to backbone mode.
        """
        if stage != 'backbone':
            print(f"[Ablation] stage='{stage}' is disabled. Redirect to stage='backbone'.")
            stage = 'backbone'
        ...
        try:
                        ...
                        if hasattr(self.model, 'latest_aux_dict'):
                            ...
                            if 'state_probs' in latest_aux:
                                ...
                                for i in range(show_n):
                                    ...
                                    sample_msgs.append(
                                        f"id={sid}, "
                                        f"q=[{', '.join(f'{v:.3f}' for v in qi.tolist())}], "
                                        f"argmax={assign_i}, "
                                        f"qmax={qmax_i:.3f}"
                                    )
            ...
            print(
                f"[Train Error] Stage={stage}, "
                f"Epoch {self.current_epoch} failed: {str(e)}"
            )
            raise
    ...
    def valid(self, dataModule, stage='backbone'):
        ...
        stage='gate' is disabled in this ablation.
        """
        if stage != 'backbone':
            print(f"[Ablation] valid stage='{stage}' is disabled. Redirect to stage='backbone'.")
            stage = 'backbone'
    ...
    def test(self, dataModule):
        ...
        # 保存测试集真实序列和预测序列用于画图。
        # 如果你的测试窗口是不重叠的，这里 reshape(-1) 就可以直接作为完整曲线。
    ...
    def _run_backbone_gate_once(self, config, runId, model, datamodule, log):
        ...
        Removed:
            1. prepare_retrieval_index()
            2. gate training
            3. gate validation
            4. setup_gate_optimizer()
            5. mark_gate_ready(True)

        Final pipeline:
            State Prior Pretraining
                ↓
            Backbone Training
                ↓
            Validation
        ...
        # ============================================================
        # 1. If checkpoint exists and retrain is not required,
        #    load checkpoint and test directly.
        #    Retrieval index construction is removed.
        # ============================================================
        if not retrain_required:
            try:
                metrics_path = f'./results/metrics/' + log.filename + '.pkl'

                sum_time = pickle.load(
                    open(metrics_path, 'rb')
                )['train_time'][runId]
                ...
                # Ablation: no retrieval index construction.
        ...
        # ============================================================
        # 2. Continue training from checkpoint if required.
        # ============================================================
        if config.continue_train and os.path.exists(model_path):
        ...
        if retrain_required:
            ...
            for epoch in trange(config.epochs, desc='Backbone Training'):
                ...
                print(
                    f"[Backbone] Current LR: "
                    f"{model.optimizer.param_groups[0]['lr']:.6g}"
                )
            ...
            # Ablation: no gate training.
            # gate_epochs = getattr(config, 'gate_epochs', 5)
            # model.model.freeze_backbone_for_gate()
            # model.setup_gate_optimizer(config)
            # model.train_one_epoch(datamodule, stage='gate')
            # model.valid(datamodule, stage='gate')
            # model.model.mark_gate_ready(True)

            model.model.unfreeze_all()

            best_epoch = getattr(monitor, 'best_epoch', len(train_time))

            if isinstance(best_epoch, int) and best_epoch > 0:
                sum_time = sum(train_time[:best_epoch])
            else:
                sum_time = sum(train_time)
    ...
    def RunOnce(self, config, runId, model, datamodule, log):
        ...
        if self._need_retrain(config, runId, log) and pretrain_epochs > 0:
            ...
            for epoch in trange(pretrain_epochs, desc='State Prior Pretraining'):
                ...
                self.model.state_prior.temperature = (
                ...
                print(
                    f"[Pretrain] Current LR: "
                    f"{self.pretrain_optimizer.param_groups[0]['lr']:.6g}, "
